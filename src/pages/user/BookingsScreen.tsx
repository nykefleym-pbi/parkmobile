...
export default function BookingsScreen() {
  ...
  function handleCancelClick(bk: Booking) {
    const tp = totalPaid(bk);
    if (tp > 0 && !isFullyPaid(bk)) {
      // Partially paid — check coverage
      const covEnd = coverageEndDate(bk);
      const todayStr = isoDate(today());
      if (todayStr < covEnd) {
        setWarnPaidId(bk.id);
        return;
      }
    } else if (isFullyPaid(bk)) {
      setWarnPaidId(bk.id);
      return;
    }
    setCancelId(bk.id);
  ...
  return (
    <div className="pa-screen-content">
      ...
      {bookings.slice().reverse().map((bk, i) => {
        ...
        return (
          <div key={bk.id} className={`pa-bk-card pa-fu pa-d${Math.min(i + 2, 5)}`}>
            <div className="pa-bk-top" onClick={() => setExpandedId(isExpanded ? null : bk.id)} style={{ cursor: 'pointer' }}>
              ...
              <>
                ...
                {isC && rem > 0 && (
                  <div className="pa-penalty-warn">
                    ...
                    <div style={{ color: 'var(--pa-tx2)' }}>
                      {pen > 0 ? 'Prorated fee + overstay penalty fee' : 'Prorated fee'} of <strong style={{ color: 'var(--pa-red)' }}>{formatPeso(owed)}</strong> must still be settled. Balance: <strong style={{ color: 'var(--pa-red)' }}>{formatPeso(rem)}</strong>
                    </div>
                  </div>
                )}
                ...
                {isA && (
                  <div className="pa-bk-actions">
                    ...
                    <button className="pa-bk-btn danger" onClick={() => handleCancelClick(bk)}>Cancel</button>
                  </div>
                )}
              </>
            )}
      ...
      {/* Paid booking warning modal */}
      {warnPaidId && (
        <div className="pa-modal-bg show" onClick={() => setWarnPaidId(null)}>
          <div className="pa-modal" onClick={e => e.stopPropagation()}>
            <h3>Cannot Cancel</h3>
            <p>Paid bookings can only be cancelled on or after the coverage period has lapsed. This prevents refund complications.</p>
            <div className="pa-modal-btns">
              <button className="pa-m-cancel" onClick={() => setWarnPaidId(null)}>Understood</button>
            </div>
          </div>
        </div>
      )}
